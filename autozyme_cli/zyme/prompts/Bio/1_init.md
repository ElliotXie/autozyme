...
Estimate baseline wall time from the source. Sketch the dominant work term as a function of dataset size, plug in typical medium-tier values (5–10K cells/spots, default params) for an order-of-magnitude. If the estimate is well below 30s on a small-tier dataset (the lower edge of step 2's target band), stop and ask the user whether to switch target — the function may not have meaningful CPU headroom.
...
- small: ideally 30s < time ≤5 min;
- medium: 1 min <time ≤15 min;
- large: 2 min < time <≤ 30 min.
...
2. Else if `<workspace>/datasets/README.md` exists, pick from the matching category dir (`single_cell/`, `spatial/corpus/`, `bulk/`) and symlink into `data/` (use full category paths — no flat files at `datasets/` root).
...
- repeating/tiling rows/cells/spots,
...
When upstream needs its own env, provision a dedicated conda env, install upstream + `scalene` + `memray`, declare via `executor:`, document in `setup/README.md`.

For BLAS-implicit targets (numpy/scipy: eigh, SVD, GEMM, lstsq), pin `OMP_/OPENBLAS_/MKL_NUM_THREADS` from `ZYME_THREADS` at the top of both `reference.{py,R}` and `pipeline/run.{py,R}`, before `import numpy`. Multithreaded BLAS isn't bit-reproducible; parity fails by ~1e-4 without this.
...
- one-paragraph summary: target function, tier datasets, measured baseline times, cell counts, key metrics and thresholds;
...
- Do not install packages during **iterate**; flag missing. Init may provision a dedicated env (see §2).
