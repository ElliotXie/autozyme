...
You are a genius computational biologist. This task may have an unfair-baseline bug; your job is one round of audit, diagnose, and (if applicable) retrofit. Do not iterate, do not start a new optimization round.

## The pattern

`reference.{R,py}` was written without threading. Later in iteration someone added a parallel construct inside `pipeline/run.{R,py}` — `mclapply`, `BPPARAM`, `future`, `joblib`, `multiprocessing`, …. Result: optimized side gets N cores, baseline gets 1. `speedup_pct` mixes algorithmic gains with raw parallelism — the headline is dishonest by roughly a factor of N.
...
- `pipeline/run.{R,py}` has no threading: grep `mclapply|parallel::|BPPARAM|MulticoreParam|future|furrr|foreach|%dopar%|joblib|n_jobs|num_workers|num_threads|mc.cores|OMP_NUM_THREADS|RcppParallel::setThreadOptions|numba.set_num_threads`.
...
R: `parallel=TRUE` + `BPPARAM=MulticoreParam(N)`, `mc.cores=`, `nthreads=`. Python: `n_jobs=`, `num_workers=`, env vars like `OMP_NUM_THREADS` actually consumed by the function.
...
if (N > 1L) {
    ...
    fitted <- lapply(ddsl, function(dds) DESeq(dds, parallel = TRUE, quiet = TRUE))
} else {
    fitted <- lapply(ddsl, function(dds) DESeq(dds, quiet = TRUE))
}
```
