...
Read: `task.yaml`, `memory/best_state.md`, `memory/active_opts.md`, `memory/discoveries.md`, `memory/dead_ends.md`, `results.tsv`. Identify the production tier (largest baseline), the target function signature + language, and a one-line field/domain tagline (e.g. "R · Bioconductor · single-cell CNV inference").
...
(e.g. R→C++ port + parallelize). Use `FLAG: <self-label>` for genuinely
out-of-taxonomy rounds (build fixes, portability, measurement-only).
