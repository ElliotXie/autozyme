...
- **Fallbacks:** explicit `clusters`, non-`dgCMatrix` `toc`, `method != "subtraction"`, `roundToInt = TRUE`, non-default `tol`/`pCut`, extra `...`, and `zyme = FALSE` delegate to upstream SoupX.
- **Native kernels:** `src/soupx.cpp` implements exact water-filling allocation over sparse CSC slots for clustered and no-cluster subtraction.
